...
function setupEventListeners() {
    ...
    // Review form artık comments.js tarafından yönetiliyor
...
function showBookDetails(bookId) {
    ...
    window.bookApi.getBookDetails(bookId)
        .then(book => {
            ...
            // Backend API'den yorumları yükle
            window.commentSystem.loadBookComments(bookId);
            
            // Yorum formunu ayarla
            window.commentSystem.setupReviewForm(bookId);
...
function showCategoryBooks(category) {
    ...
    document.querySelector('#search-results-section h2').textContent = `${categoryTitle} Kitapları`;
...
function getCategoryTitle(category) {
    ...
    return categories[category] || 'Kategori';
}
...
function loadFeaturedBooks() {
    ...
    window.bookApi.getFeaturedBooks()
        ...
        .catch(error => {
            ...
            featuredBooksContainer.innerHTML = `<div class="col-12"><div class="alert alert-danger">Öne çıkan kitaplar yüklenirken bir hata oluştu: ${error.message}</div></div>`;
        });
}
...
function displayFeaturedBooks(books) {
    ...
    if (!books || books.length === 0) {
        featuredBooksContainer.innerHTML = '<div class="col-12"><div class="alert alert-info">Öne çıkan kitap bulunamadı.</div></div>';
        return;
    }
...
// Reading list functionality from reading-list.js
function updateBookDetailsReadingListUI(bookId) {
    // Check if the book is in reading list
    const isInReadingList = window.readingList && window.readingList.isInReadingList(bookId);
    
    // Get the book info container
    const bookInfoContainer = document.getElementById('book-info-container');
    
    // Remove existing reading list button if any
    const existingButton = bookInfoContainer.querySelector('.reading-list-btn');
    if (existingButton) {
    ...
    readingListBtn.className = `btn ${isInReadingList ? 'btn-danger' : 'btn-success'} reading-list-btn mt-3`;
    readingListBtn.innerHTML = isInReadingList ? 
        '<i class="fas fa-times me-2"></i>Okuma Listemden Çıkar' : 
        '<i class="fas fa-plus me-2"></i>Okuma Listeme Ekle';
    
    // Add click event
    readingListBtn.addEventListener('click', () => {
        if (isInReadingList) {
            window.readingList.removeFromReadingList(bookId);
        } else {
            window.readingList.addToReadingList(bookId);
        }
    ...
    // Add button to book info container
    const bookInfo = bookInfoContainer.querySelector('.book-info');
    if (bookInfo) {
        bookInfo.appendChild(readingListBtn);
    }
}
